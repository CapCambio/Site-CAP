...
export function CurrencyConverter({ currencies }: CurrencyConverterProps) {
  ...
  const allCurrencies = [
    ...currencies.filter(c => c.code !== "BRL"),
    { 
      code: "BRL", 
  ...
  const convertCurrency = () => {
    ...
    // Se ambas moedas forem estrangeiras, convertemos primeiro para BRL e depois para a moeda de destino
    else {
      const sourceCurrency = currencies.find(c => c.code === fromCurrency);
      const targetCurrency = currencies.find(c => c.code === toCurrency);
      ...
      const brlValue = Number(amount) * sourceCurrency.buyPrice;
      // Depois converter de BRL para a moeda de destino
      result = brlValue / targetCurrency.sellPrice;
    }
  ...
  return (
    <div className="relative max-w-xl mx-auto my-6">
      <div className="bg-[#252525] p-4 sm:p-6 rounded-xl overflow-hidden">
        ...
        <div className="bg-[#f3b234] rounded-xl flex justify-between items-center p-4 mb-4 relative">
          <input
            type="text"
            value={amount}
        ...
        <div className="bg-[#f3b234] rounded-xl flex justify-between items-center p-4 mt-3 relative">
